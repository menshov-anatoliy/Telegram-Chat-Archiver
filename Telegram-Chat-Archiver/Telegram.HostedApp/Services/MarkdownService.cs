...
public class MarkdownService : IMarkdownService
{
    ...
    public MarkdownService(ILogger<MarkdownService> logger, IOptions<ArchiveConfig> config)
    {
        _logger = logger;
    ...
    public async Task SaveMessagesAsync(IEnumerable<ChatMessage> messages, string chatTitle, DateTime date, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogInformation("Сохранено {MessageCount} сообщений в файл: {FilePath}", messages.Count(), filePath);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Ошибка при сохранении сообщений в Markdown файл для чата {ChatTitle}", chatTitle);
            throw;
        }
    }
    ...
    public async Task AppendMessageAsync(ChatMessage message, string chatTitle, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError(ex, "Ошибка при добавлении сообщения {MessageId} в Markdown файл", message.Id);
            throw;
        }
    }
